...
import GetPropertyRequestsFiltered from '../../application/usecases/GetPropertyRequestsFiltered';

@Service({ global: true })
export default class GetPropertyRequestsFilteredController extends HttpProcessor {
  constructor(private readonly getPropertyRequestsFiltered: GetPropertyRequestsFiltered) {
    super();
  }
  ...
  async execute(req, validatedValue): Promise<HttpSuccess | HttpError> {
    try {
      ...
      const result = await this.getPropertyRequestsFiltered.execute(request);
